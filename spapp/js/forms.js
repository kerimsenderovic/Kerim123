...
$("#tutorial-form").validate({
    ...
    submitHandler: function (form, event) {
        ...
        $.post(
            Constants.API_BASE_URL + "add_member.php",
            data,
            function(response) {
                users.push(data);
...
var MemberService = {
    reload_member_datatable: function () {
        Utils.get_datatable(
            "#tutorial-table",
            Constants.API_BASE_URL + "get_members.php",
            [
                { data: "action" },
                { data: "id" },
                { data: "firstName" },
    ...
    open_edit_member_modal: function (member_id) {
        RestClient.get("get_member.php?id=" + member_id, function (data) {
            $("#add-patient-modal").modal("toggle"); // Corrected modal selector
            $("#id").val(data.id); // Updated input field names if necessary
    ...
    delete_member: function (member_id) {
        if(confirm("Do you want to delete member with the id "+ member_id + "?")){
            RestClient.delete(
                "delete_member.php?id="+ member_id,
                {},
                function(data){
                    MemberService.reload_member_datatable();
                }
